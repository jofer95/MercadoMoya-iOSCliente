...
{
    public partial class ArticuloDetalleCarritoController : UIViewController
    {
        Servicios servicios = new Servicios();
        private Pedido pedidoRespuesta;
        public ArticuloDetalleCarritoController(IntPtr handle) : base(handle)
        {
        }
        public override void ViewDidLoad()
        {
            ...
            if (articulo != null)
            {
                ...
                var dataTask = session.CreateDataTask(new NSUrlRequest(new NSUrl(articulo.ImagenURL)), (data, response, error) =>
                {
                    if (response != null)
                    {
                        ...
                        {
                            imgArticuloCarrito.Image = UIImage.LoadFromData(data);
                        });
                    }
                });
            ...
            btnQuitarArt.TouchUpInside += delegate
            {
                ...
                ViewController.pedidoIDStatic = pedidoRespuesta.PedidoID;
                this.PerformSegue("segueAtrasQuitarCarrito", this);
